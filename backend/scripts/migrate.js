...
class MigrationRunner {
    ...
    discoverMigrations() {
        ...
        console.log(chalk.green(`✅ Found ${this.migrations.length} migration(s):`));
        this.migrations.forEach((m) => {
            const icon = m.type === 'schema' ? '📋' : m.type === 'seed' ? '🌱' : '⚡';
    ...
    async executeMigration(migration) {
        ...
        const icon = migration.type === 'schema'
            ? '📋'
            : migration.type === 'seed'
                ? '🌱'
                : '⚡';
        try {
            ...
            const isIdempotentError = idempotentErrors.some(msg => errorMsg.toLowerCase().includes(msg.toLowerCase()));
    ...
    async verifyInstallation() {
        ...
        for (const check of checks) {
            try {
                ...
                console.log(`${status} ${check.name}: ${chalk.cyan(count)} (expected ≥ ${check.expectedMin})`);
            }
            catch (error) {
                console.log(`❌ ${check.name}: ${chalk.red(error.message)}`);
            }
        }
